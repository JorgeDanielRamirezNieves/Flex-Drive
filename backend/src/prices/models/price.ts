...
export class Price {
    ...
    @ManyToOne(() => Vehicle, (objVehicle: Vehicle) => objVehicle.pricesVehicle, {
        onUpdate: 'CASCADE',
        onDelete: 'RESTRICT',
      })
      @JoinColumn([{ name: 'id_vehicle', referencedColumnName: 'uuid' }])
      public vehiclePrice?: Vehicle; 

    constructor(uuid: string, precio: number, startDate: Date, endDate: Date, idVehicle: string) {
        this.uuid = uuid;
        this.precio = precio;
